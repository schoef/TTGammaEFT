...
def getReweightingFunction(data="PU_2100_XSecCentral", mc="Spring15"):
    ...
    # Data
    fileNameData = "$CMSSW_BASE/src/TTGammaEFT/Tools/data/puReweightingData/%s.root" % data
    ...
    if type(mc)==type(""):
        if mc=='Summer16':
            mcProfile = extendHistoTo(getObjFromFile("$CMSSW_BASE/src/TTGammaEFT/Tools/data/puReweightingData/MCProfile_Summer16.root", 'pileup'), histoData)
        else:
...
def getNVTXReweightingFunction(key, filename = "dilepton_allZ_isOS_4000pb4000_80X.pkl"):
    ...
    import pickle, os
    data = pickle.load( file(os.path.expandvars("$CMSSW_BASE/src/TTGammaEFT/Tools/data/puReweightingData/%s" % filename)) )
    h = data[key]
    def reweightingFunc( nvtx ):
        ib = h.FindBin( nvtx )
