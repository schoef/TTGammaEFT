...
puDataPath = "$CMSSW_BASE/src/TTGammaEFT/Tools/data/puReweightingData/"

def extendHistoTo(h, hc):
    logger.info( "Extend histo h to nbins of hc" )
...
def getReweightingFunction( data="PU_2100_XSecCentral", mc="Spring15" ):
    ...
    if isinstance( mc, basestring ):
        if mc=='Summer16':
            ...
            mcProfile = extendHistoTo( getObjFromFile( os.path.expandvars( puDataPath + "MCProfile_Autumn18.root" ), 'pileup' ), histoData )
        else:
...
def getNVTXReweightingFunction( key, filename="dilepton_allZ_isOS_4000pb4000_80X.pkl" ):
    ...
    import pickle, os
    data = pickle.load( file( os.path.expandvars( puDataPath + filename ) ) )
    h = data[key]
    def reweightingFunc( nvtx ):
        ib = h.FindBin( nvtx )
